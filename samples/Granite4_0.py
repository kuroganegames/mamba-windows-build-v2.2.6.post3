...
import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), "unsloth_compiled_cache"))
...
def main():
    ...
    # Merge to 16bit
    import os
    dir_save_base = r"I:\dev\mamba_test1\granite_unsloth_test1"
    dir_save_model = os.path.join(dir_save_base, "model")
    dir_save_lora = os.path.join(dir_save_base, "lora")
    ...
    # Just LoRA adapters
    if bSaveModelOnly:
        model.save_pretrained(dir_save_lora)
...
if __name__ == "__main__":
    ...
    mp.freeze_support()  # 付けても害はない（Windows向け）
    main()